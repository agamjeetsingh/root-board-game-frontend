//
// Created by Agamjeet Singh on 20/03/2026.
//

#ifndef ROOT_BOARD_GAME_FRONTEND_UIMANAGER_H
#define ROOT_BOARD_GAME_FRONTEND_UIMANAGER_H

#include "Widget.h"

class UIManager {
public:
    explicit UIManager(EventBus& event_bus): widget(std::make_unique<Widget>(std::make_unique<RectHitbox>(sf::Vector2f{0, 0}, sf::Vector2f{MAXFLOAT, MAXFLOAT}))),
    clickListener(Listener::make_listener<sf::Event::MouseButtonPressed>([this](const sf::Event::MouseButtonPressed& e) {
        widget->onClick(e);
    }, event_bus)),
    mouseMovementListener(Listener::make_listener<MousePosition>([this](const MousePosition& e) {
        widget->onHover(e);
    }, event_bus)) {}

    void addChild(std::unique_ptr<Widget> child) const {
        widget->addChild(std::move(child));
    }

    void draw(sf::RenderTarget& target) const {
        widget->draw(target, ORIGIN);
    }

    void update(float dt) const {
        widget->update(dt);
    }

private:
    std::unique_ptr<Widget> widget;

    std::optional<Widget*> widgetUnderMouse;

    Listener clickListener;

    Listener mouseMovementListener;

    sf::Vector2f ORIGIN{0, 0};
};

#endif //ROOT_BOARD_GAME_FRONTEND_UIMANAGER_H